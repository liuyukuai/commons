...
public class SuspendableThreadPoolExecutor extends ThreadPoolExecutor {
    ...
    protected void beforeExecute(final @NonNull Thread thread, final @NonNull Runnable task) {
        ...
        log.info("job task pool get available is = {}", available);
        suspendLock.lock();
        try {
            while (!available) {
                log.info("job task pool will awaited");
    ...
    public void setAvailable(final boolean available) {
        ...
        try {
            ...
            log.info("job task pool set available is = {}", this.available);
            if (available) {
                log.info("job task pool will signalAll");
